...
public class IoWrapper : IIoWrapper
{
	...
	{
		using var trace1 = Tracing.Trace(nameof(MkDirIfNotExists))
									.WithTag("path", path);
		if (!DirExists(path))
		{
			using var trace2 = Tracing.Trace("CreateDirectory")
										.WithTag("path", path);
	...
	{
		using var trace1 = Tracing.Trace(nameof(DirExists))
									.WithTag("path", path);
	...
	{
		using var trace1 = Tracing.Trace(nameof(GetFiles))
									.WithTag("path", path);
	...
	{
		using var trace = Tracing.Trace(nameof(MoveFailedFile))
	...
	{
		using var trace1 = Tracing.Trace(nameof(Copy))
	...
	{
		using var trace = Tracing.Trace(nameof(WriteToFile))
									.WithTag("path", path);
		try
		{
	...
	{
		using var trace = Tracing.Trace(nameof(Cleanup))
									.WithTag("path", dir);
		if (!DirExists(dir))
			return;

		using var trace2 = Tracing.Trace("DeleteDir")
										.WithTag("path", dir);
